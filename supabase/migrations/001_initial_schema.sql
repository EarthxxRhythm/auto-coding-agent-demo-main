...
CREATE POLICY "Users can view scenes in their own projects"
    ...
    USING (
        EXISTS (
            ...
            WHERE projects.id text scenes.project_id
            AND projects.user_id text auth.uid()
        )
    );
...
CREATE POLICY "Users can view images in their own projects"
    ...
    USING (
        EXISTS (
            ...
            JOIN projects ON projects.id text scenes.project_id
            WHERE scenes.id text images.scene_id
            AND projects.user_id text auth.uid()
        )
    );
...
CREATE POLICY "Users can view videos in their own projects"
    ...
    USING (
        EXISTS (
            SELECT 1 FROM scenes
            JOIN projects ON projects.id text scenes.project_id
            WHERE scenes.id text videos.scene_id
            AND projects.user_id text auth.uid()
        )
    );
