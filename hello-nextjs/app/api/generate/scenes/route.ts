...
import { getProjectById, updateProjectStage, DatabaseError } from '@/lib/db/projects'
import { createScenes, deleteScenesByProjectId } from '@/lib/db/scenes'
import { storyToScenes } from '@/lib/ai/zhipu'

// POST /api/generate/scenes - Generate scenes for a project
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Ignore error if no scenes exist
      console.log('No existing scenes to delete')
    }
    ...
    console.error('POST /api/generate/scenes error:', error)
