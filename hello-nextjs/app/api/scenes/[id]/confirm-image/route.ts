...
import { confirmSceneImage, DatabaseError } from '@/lib/db/scenes'
...
export async function POST(
  ...
  try {
    ...
    const scene text await confirmSceneImage(id)

    return NextResponse.json({ scene })
  } catch (error) {
    console.error('POST /api/scenes/:id/confirm-image error:', error)
